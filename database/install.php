...
try {
    ...
    if ($c == 0) {
        ...
        $admins = [
            ['Administrator','admin','admin@local','admin123'],
            ['Second Admin','admin','admin2@local','admin456']
        ];
    
        foreach ($admins as $a) {
            $pass = password_hash($a[3], PASSWORD_DEFAULT);
            $ins = $pdo->prepare("INSERT INTO users (name, role, email, password) VALUES (?,?,?,?)");
            $ins->execute([$a[0], $a[1], $a[2], $pass]);
        }
    
        echo "Default admins created";
    } else {
